...
// Check if image file is a actual image or fake image
  ...
  if($check !== false) {
    ...
    header('location:category_add.php?msg_image_not=success');
...
if (file_exists($target_file)) {
    header('location:category_add.php?msg_image_already_exists');
  $uploadOk = 0;
}

// Check file size
if ($_FILES["cat_image"]["size"] > 500000) {
    header('location:category_add.php?msg_image_too_large=success');
  $uploadOk = 0;
}
...
&& $imageFileType != "gif" ) {
    header('location:category_add.php?msg_image_format_mismatch=success');
  $uploadOk = 0;
}
...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["cat_image"]["tmp_name"], $target_file)) {
    ...
    $sql="INSERT INTO `category_ecom`(`c_name`, `c_desc`, `c_date`, `c_status`, `c_image`) VALUES ('$name','$desc','$today','$status','$target_file')";
    $conn->query($sql);
            header('location:category_add.php?msg_pass=success');
  } else {
    header('location:category_add.php?msg_fail=success');
  }
}
}
?>
...
<div class="inner-block">
    <div class="blank">
        ...
        <div class="list_cat">
            <a href="category_list.php">list category</a>
        </div>
...
?>
      ...
      <div class="child">
        <form action="category_add.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="adjust">
                <input type="text" placeholder="category name" name="cat_name" style="border-radius: 5px;" class="textstyle" required><br>
                <input type="text" placeholder="category description" name="cat_desc" style="border-radius: 5px;" class="textstyle" required>
                ...
                <input type="file" placeholder="select image" name="cat_image" required><br>
